(function () {
  ...
  /* ---------- Selected Work — 3D project carousel ----------
     ...
     markup rewrite — see item 13 of this round's brief). The 3D stage
     (.work-slide, image only) and the single external #work-info block are
     kept in sync from the same active index. Only the active slide's image
     participates in interaction (pointer-events on neighbors are off in
     CSS); #work-info is normal-flow, so it can never overflow a fixed-height
     3D box and collide with the nav below it — that was the actual bug
     behind the "controls too close to the content" issue this round is
     fixing. Arrow keys work once the carousel region has focus; touch
     devices get horizontal swipe. No autoplay. */
  var workCarousel = document.getElementById('work-carousel');
  if (workCarousel) {
    var WORK_PROJECTS = [
      { title: 'Project 01', category: 'Website Design + Development',
        desc: 'A full presence rebuild — structure, speed, and content working as one system.' },
      { title: 'Project 02', category: 'UI / UX Design',
        desc: 'A wireframe-first redesign that resolved the whole flow before a single pixel shipped.' },
      { title: 'Project 03', category: 'SEO & Growth',
        desc: 'A 40-page migration engineered for indexing and load time from day one.' },
      { title: 'Project 04', category: 'Website Design + Development',
        desc: 'A booking-flow rebuild — from first visit to confirmed reservation, one connected system.' }
    ];
    ...
    function paintWorkInfo() {
      ...
      if (workInfoCategory) workInfoCategory.textContent = p.category;
      if (workInfoDesc) workInfoDesc.textContent = p.desc;
    ...
    // The placeholder CTA has nowhere real to go yet — prevent the "#" jump
    // rather than leave a broken/confusing link; trivial to remove once a
    // real project URL replaces href="#work-carousel".
    var workInfoCta = document.getElementById('work-info-cta');
    if (workInfoCta) workInfoCta.addEventListener('click', function (e) { e.preventDefault(); });

    // Init directly — no fade, no timeout — the static HTML already shows
    // Project 01's text, so this just sets the initial 3D offsets to match.
    updateOffsets();
    paintWorkInfo();
  }
  ...
  /* ---------- Contact form ----------
     HONEST submission architecture — no faked success. Previously this used
     setTimeout() to simulate a delay and then unconditionally displayed
     "Message sent," regardless of whether any request actually happened.
     That's gone. Real states: idle -> submitting -> success (only after an
     actual successful response) or error (shown for both a failed request
     AND for the case below, where no real endpoint is configured yet).

     ============================================================
     SINGLE INTEGRATION POINT: set CONTACT_FORM_ENDPOINT below to a real
     form-submission service or serverless function that forwards to
     chaperslab@gmail.com (e.g. Formspree, Getform, a small serverless
     function calling a transactional email API). No email credentials,
     SMTP secrets, or API keys can live in this client-side file — whatever
     service is used must handle delivery server-side. Until this constant
     is set, submissions correctly show the error state below rather than a
     fake confirmation, since there is genuinely nowhere for them to go yet.
     ============================================================ */
  var CONTACT_FORM_ENDPOINT = '';

  // The only runtime-generated, user-facing strings in this file — status/
  // error text that JS itself decides when to show, so it can't live as
  // static translated HTML the way the rest of the page's copy does. Not a
  // page-content translation system: just the small set of dynamic form
  // messages, keyed off document.documentElement.lang.
  var FORM_STRINGS = {
    es: {
      sending: 'Enviando…',
      invalid: 'Por favor completa todos los campos requeridos correctamente.',
      noEndpoint: 'Algo salió mal. Inténtalo de nuevo, o escríbenos directamente.',
      requestError: 'Algo salió mal. Inténtalo de nuevo.',
      success: 'Mensaje recibido. Nos pondremos en contacto contigo pronto.'
    },
    en: {
      sending: 'Sending…',
      invalid: 'Please fill in all required fields correctly.',
      noEndpoint: 'Something went wrong. Please try again, or email us directly.',
  ...
  if (form) {
    ...
    function setSubmitting(on) {
      ...
      if (on) status.textContent = formStrings.sending;
    }
    ...
    form.addEventListener('submit', function (e) {
      ...
      if (!CONTACT_FORM_ENDPOINT) {
        // No real delivery mechanism is connected yet — this is the honest
        // outcome, not a fake success. See the note above this block.
        showError(formStrings.noEndpoint);
        return;
      }
